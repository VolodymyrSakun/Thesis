...
def plot_partial_effects_on_outcome(featuresPlotDict, fitter, suffix):                
    M = 2
    N = 2
...
def boxPlotSurvivalTime(models, fitResultsDict, suffix, p=None):
    tExpectedList = []
    tMedianList = []    
    maxExpected = -np.inf
    minExpected = np.inf
...
def plotSurvivalSample(sModel, sampleId, S_Large, baseline=None):
...
def plotCorrelation(c, cmap='BuPu'):
    fig, ax = plt.subplots(1, 1, figsize=(19,15))
...
#   AFT
    ...
    argsDict = {'RandomSurvivalForest': 
                ...
                'WeibullAFT': {'penalizer': 0.1},
                'LogNormalAFT': {'penalizer': 0.01},
                'LogLogisticAFT': {'penalizer': 0.1},
    ...
    for sModel in tqdm(modelsRegression):
        ...
        if sModel in ['WeibullAFT', 'LogNormalAFT', 'LogLogisticAFT', 'CoxPHFitter', 'AalenAdditiveFitter']:
            ...
            if sModel in ['WeibullAFT', 'LogNormalAFT', 'LogLogisticAFT', 'CoxPHFitter']:
                ...
                plot_partial_effects_on_outcome(featuresPlotDict, model.fitter, sModel)
...
#   S_Large is for estimating expected / median survival times
        ...
        if sModel == 'CoxPHFitter':
            plotSurvivalSample(sModel, sampleId, S_Large, baseline=model.fitter.baseline_survival_.squeeze())
        else:
...
#   Simulated data
    ...
    # box plot
    boxPlot_IAE_ISE(modelsRegression, fitResultsDict, 'Regression')
    boxPlotSurvivalTime(modelsRegression, fitResultsDict, 'Regression', p=0.99)

#   Brier scores plot
    plotBrierScore(modelsRegression, fitResultsDict, 'Regression')

# plots errors remaining life on simulated data; on real data does nothing
    boxPlotError(modelsRegression, fitResultsDict, 'Regression')
...
#   Predict future profit for population
    ...
    for sModel in models:
        ...
        for i, tRemaining in enumerate([tRemainingExpectedCensoredList, tRemainingMedianCensoredList]):
            ...
            dLastChurn = xCensoredDf['dEndBuy'].max()
