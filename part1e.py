...
if __name__ == "__main__":
    ...
    for sData in dirsList:   
        ...
        predictDf = lib2.loadObject(os.path.join(resultsDir, sData, 'dfPredict.dat'))
        predictDf['simulation'] = sData
        scoresList.append(predictDf)

    predictDf = pd.concat(scoresList, axis=0)
    
    # scatter predicted vs. true
    fig1, ax1 = plt.subplots(1, 1, figsize=(19,15))

    _ = ax1.scatter(predictDf['remainingTrue'].values, predictDf['remainingPredicted'].values, s=1)
    
    ax1.set_xlabel('True remaining life, weeks', fontsize=14)
    ax1.set_ylabel('Predicted remaining life, weeks', fontsize=14)

    title1 = 'Scatter plot predicted vs. true remaining life RNN'
    fig1.suptitle(title1, fontsize=16)

    fileName1 = '{}.png'.format(title1)
    plt.savefig(os.path.join(resultsDir, fileName1), bbox_inches='tight')
    plt.close() 
    
    
    
    # box plot true and predicted remaining life
    fig2, ax2 = plt.subplots(1, 1, figsize=(19,15))
    columns = ['remainingTrue', 'remainingPredicted', 'errorAbsWeeks']
    ax2.boxplot([predictDf['remainingTrue'].values, 
                 predictDf['remainingPredicted'].values,
                 predictDf['errorAbsWeeks'].values], vert = 1)

    ax2.set_xticklabels(columns, rotation=0, fontsize=14)
    ax2.set_ylabel('Weeks', fontsize=14)

    title2 = 'Box plot of distributions of predicted, true remaining life and absolute error'
    fig2.suptitle(title2, fontsize=16)
                
    fileName2 = '{}.png'.format(title2)
    plt.savefig(os.path.join(resultsDir, fileName2), bbox_inches='tight')
    plt.close() 
    
    predictDf['errorAbsWeeks'].mean()
    predictDf['errorAbsWeeks'].median()
    
    
    
    # g1 = predictDf.groupby('remainingTrue').agg({'errorAbsWeeks': 'mean'})
    # x1 = np.arange(0, g1['errorAbsWeeks'].max())
    # g2 = predictDf.groupby('remainingTrue').agg({'errorAbsWeeks': 'median'})
    
    # g3 = predictDf.groupby('remainingTrue').agg({'remainingPredicted': 'mean'})
    # plt.plot(g3)
    # plt.plot(x1, x1)
    ...
    # g4 = g3.iloc[0:50]
    # x2 = np.arange(0, g4['remainingPredicted'].max())
    ...
    # scores1Df = lib2.loadObject(os.path.join(workDir, 'results', 'Simulated1', 'dfPredict.dat'))
